...
# 「粒度」应写「以 X 为单位」或「每 N 一次」;「对齐」在非内存语境下应写「解析到」
# 「统一到」——「内存对齐」「字节对齐」是硬件术语,由 EXCLUSIONS 保留。
BLOCK_TERMS: tuple[str, ...] = (
    ...
    "粒度",
    "对齐",
    # 规范 1.8:破折号。中文破折号写作两个全角横线,单个的多为误用,两种都拦。
    "——",
    "—",
)
...
# 等义,两种写法都可接受,因此归入提示级。
# 「链路」与「收敛」留在提示级的理由与其余词不同:它们在本项目里主要是网络与路由的
# 标准译名(链路本地地址、路由收敛),正当用法无法用有限白名单穷举。
