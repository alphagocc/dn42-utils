...
- **稳态下绝大多数请求是空转。** 配置没变时每次 pull 仍要跑一次 `build_desired_state`（含一次 `config_revisions` 写入）+ 一次 argon2id 全表扫描鉴权。
...
**argon2 每连接只算一次。** `ManagedNodeStore.authenticate` 是对所有 enabled 节点线性扫描 + 逐行 argon2id 验证（约 50–100 ms/行）。握手时验一次，把结果 `Principal` 缓存到连接对象上，之后每一帧的鉴权纯粹读缓存，零 DB、零 argon2。
...
采用 full jitter 的原因在于，主要失效模式就是 **hub 重启后全队同步重连**：`authenticate` 的线性扫描让 N 个节点同时重连产生 O(N²) 次 argon2 验证。full jitter 把这个尖峰摊得最平。

鉴权类致命关闭（`4003` / `4004` / `4008` / `4401` / `4403` / `4404`）改用固定的 `auth_retry_seconds`（默认 300s）：指数爬坡会让一个过期 token 演变成对 hub 的 1 秒间隔 argon2 DoS。
...
现有 service 层全部是同步阻塞的（`build_desired_state`、`submit_proposal`、`submit_report`、`authenticate` 等），而 WS 端点必须是 `async def`。所有这些调用一律放到线程池执行：在事件循环里直接运行时，一次 argon2 验证就会卡住整个 hub 的所有连接。
...
- **`authenticate` 是 O(nodes) 线性扫描。** WS 把这个成本从"摊薄在每 10 分钟一次"变成了"hub 每次重启时的 O(N²) 突发"。当前缓解是 full jitter 退避 + hub 侧握手信号量限流。根治方案是给 token 加一个非敏感的 id 前缀做 O(1) 查找，涉及 token 格式与 schema 变更，**不在本次范围**。
