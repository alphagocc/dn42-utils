...
3. `managed_nodes` UPSERT：先把其他行的 `is_self` 清零，再写入 `(node_id=<self>, name='self', is_self=1, enabled=1, write_policy=<默认 JSON>)`。清零这一步不可省：`self_node_id` 文件丢失后会生成新 UUID，只写不清就会留下两行 `is_self=1`，而 `get_self()` 无从判断该返回哪一行。旧行降级为普通受管节点，它的 peer 一条不动，可用 `dn42ctl node adopt-self` 搬迁。
   - **`name` 只在这一行第一次创建时写入。** 该行已存在时 UPSERT 只刷新 `is_self`、`enabled` 与 `updated_at`。每次启动都会执行这一步，连名字一起写回默认值会把运维的改名撤销掉，而这个名字正是公共 auto-peer 页面上展示的内容，详见 [`auto_peer.md`](auto_peer.md)。
...
- `/etc/dn42ctl/node.toml`（`0600`）：server URL / node_id / token / `[apply] reload` / `[agent]` 调优参数。
- `/var/lib/dn42ctl/node-cache.sqlite3`：缓存最近 desired state 与 revision。**仅缓存**，丢失不影响权威状态。agent 启动时会先用这份缓存执行一次 `apply()`，然后才尝试连接，以此保证 hub 不可达时 `/etc/bird` 仍会被渲染。该设计的完整论证见 `docs/architecture/sync_ws_protocol.md`。
