...
权限在 `src/dn42ctl/file_policy.py` 统一声明，写入方均从那里取，详见 [`paths.md`](paths.md)。
...
目前 `dn42-dummy` 是唯一同时支持 `networkd` 与 `nm` 两种后端的接口，由 `config.toml` 的 `dummy_backend` 字段选择。创建行为与失败处理详见 [`../commands/init.md`](../commands/init.md)。
