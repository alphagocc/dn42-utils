...
| 方法 | 路径 | 对应 CLI | 说明 |
|------|------|---------|------|
| `GET` | `/api/admin/bgp/peers` | `show bgp` | 列出所有 BGP peer（支持 `?live=false` 跳过探测） |
| `POST` | `/api/admin/bgp/peers` | `bgp peer` | 创建 BGP peer |
| `PUT` | `/api/admin/bgp/peers/{asn}` | `bgp peer modify` | 修改 BGP peer |
| `DELETE` | `/api/admin/bgp/peers/{asn}` | `bgp peer del` | 删除 BGP peer |
| `GET` | `/api/admin/ibgp/peers` | `show ibgp` | 列出所有 iBGP peer（支持 `?live=false`） |
| `POST` | `/api/admin/ibgp/peers` | `ibgp peer` | 创建 iBGP peer |
| `PUT` | `/api/admin/ibgp/peers/{name}` | `ibgp peer modify` | 修改 iBGP peer |
| `DELETE` | `/api/admin/ibgp/peers/{name}` | `ibgp peer del` | 删除 iBGP peer |
| `GET` | `/api/admin/wg/tunnels` | `show wg` | 列出所有 WireGuard 隧道（支持 `?live=false`） |
