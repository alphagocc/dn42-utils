...
- 尽力读取 `babel.conf` 并解析每个 `interface` 的 `rxcost` 与 `type`：
  - 若能匹配到 `wg_<name>` 接口：写入 DB 的 `ibgp_peers.babel_rxcost` / `ibgp_peers.babel_type`。
  - 若无法读取/解析或缺失该接口：记录 warning，并回退到默认值（`type` 回退为 `tunnel`），保持兼容。
  - 详见 [`../architecture/babel.md`](../architecture/babel.md)。
- 尽力从 networkd/Bird peers 中拼装出同一个 peer 的字段（例如 endpoint/keys/AllowedIPs/peer_lla 等）；缺失字段允许为空。
